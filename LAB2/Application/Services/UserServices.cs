...
public class UserServices(IUserRepository userRepository, ITokenGenerator tokenGenerator, IUnitOfWork unitOfWork) : IUserUseCases
{
    ...
    public LoginUserResult LoginUser(LoginUserDTO loginUserDTO)
    {
        ...
        if (user is not null)
        {
            if (user.Password == loginUserDTO.Password)
            {
                ...
                {
                    Status = UserLoginStatus.UserLogged,
                    Token = token
                };
            }
            else
            {
                return new LoginUserResult
                {
                    Status = UserLoginStatus.WrongUserOrPassword
                };
            }
        }
        else
